#' jaccard
#'
#' This function calculates Jaccard Index and the Jaccard Distance. 
#' @param d results of function aggregatoR
#' @param method "s" Classical Jaccard similarity; "d" Classical Jaccard distance; 
#' @param method "ras" Relative Abbundance Jaccard Similarity; "rad" Relative Abbundance Jaccard Distance;
#' @param method "cras" Correction Relative Abbundance Jaccard Similarity; "crad" Correction Relative Abbundance Jaccard Distance;
#' @keywords aggregatoR
#' @details jaccard is calculated according to Jaccard Index
#' @references Jaccard, P. 1912. The distribution of the flora in the alpine zone. New Phytologist 11: 37–50.
#' @export
...
jaccard <- function(y, method = "s"){
  ...
  for (p in 1:length(comb_pa)) {
    t <- t2.df[,p]/(t2.df[,p]+(nrow(y1)-t2.df[,p]))
    t3[p] <- t 
  }
  ...
  # UV/U+V-UV
  jac_ab_rel <- UV/(U+V-UV)
  jac_ab_rel.df <- as.data.frame(t(jac_ab_rel))
  names(jac_ab_rel.df) <- t1
  
  # Jaccard relative abbundance [output]
  jac_ab_rel.df <- jac_ab_rel.df


  # Jaccard relative abundance with factor correction
  ...
  # Jaccard index with correction [output]
  jac_correct <-vector(length = length(U))
  for (p in 1:length(V)) {
    j_cor <- (U_correct[p]*V_correct[p])/(U_correct[p]+V_correct[p]-U_correct[p]*V_correct[p])
    jac_correct[p] <- j_cor
  }
  jac_correct.df <- as.data.frame(t(jac_correct))
  names(jac_correct.df) <- t1
  ...
  if(method == "ras"){
    output <- (jac_ab_rel.df)
  }
  
  if(method == "rad"){
    output <- 1 - jac_ab_rel.df
  }
  
  if(method == "cras"){
    output <- (jac_correct.df)
  }
  
  if(method == "crad"){
    output <- 1 - jac_correct.df
  }
  return(output)
}