################################################################################
bioDB.1 <- function(x, overwrite=F){
  ...
  for (i in 1:12) {
    x_filt[ ,i][which(x_filt[ ,i] == "")] <- NA
  }

  ### Large cicle for, after checking if work we can delete that ###############
  ...
  for (i in 14:23) {
    x_filt$taxa[!is.na(x_filt[ ,i])] = x_filt[ , i][!is.na(x_filt[ ,i])]
  }
